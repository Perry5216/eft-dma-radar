...
namespace eft_dma_radar
{
    internal static class Memory
    {
        ...
        public static ReadOnlyCollection<LootItem> Loot
        {
            get => _game?.Loot;
        }
        ...
        private static void Worker()
        {
                    ...
                    while (true) // Game is running
                    {
                        ...
                        try
                        {
                            ...
                            while (_game.InGame)
                            {
                                ...
                                {
                                    Ticks = _ticks; // push count to public property
                                    _ticks = 0;
                                    _tickSw.Restart();
                                }
                                else _ticks++;
                        ...
                        {
                            Program.Log($"Unhandled exception in Game Loop: {ex}");
                        }
                        finally 
                        {
                            _ready = false;
            ...
            catch (DMAShutdown) { Program.Log("Memory Thread closing down due to DMA Shutdown..."); } // Shutdown Thread Gracefully
            catch (Exception ex)
            {
                Program.Log($"FATAL ERROR on Memory Thread: {ex}");
        ...
        public static void Shutdown()
        {
            if (_running)
            {
                Program.Log("Closing down DMA Connection...");
                _running = false;
                vmm.Close();
            }
        }
