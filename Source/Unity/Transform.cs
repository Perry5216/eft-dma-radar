...
namespace eft_dma_radar
{
	public class Transform
	{
		...
		private readonly ulong _indicesAddress;
		...
		public Transform(ulong transformInternal, bool isPlayerTransform = false)
		{
			...
			_indicesAddress = Memory.ReadValue<ulong>(hierarchy + Offsets.TransformHierarchy.Indices);

			_verticesAddress = Memory.ReadValue<ulong>(hierarchy + Offsets.TransformHierarchy.Vertices); 
        ...
        public Vector3 GetPosition(object[] obj = null)
		{
			...
			{
				indices = ReadIndices(_indicesAddress, HierarchyIndex + 1); // address, count
				vertices = ReadVertices128(_verticesAddress, 3 * HierarchyIndex + 3); // Reference v9/v10 in IDA
			}
		...
		{
			return new Tuple<ulong, int, ulong, int>(_indicesAddress,
				HierarchyIndex + 1,
				_verticesAddress,
				3 * HierarchyIndex + 3);
		}
