...
namespace vmmsharp
{
    ...
    public static class lc
    {
        ...
        public static unsafe MEM_SCATTER[] ReadScatter(ulong hLC, params ulong[] qwA)
        {
            ...
            for (i = 0; i < MEMs.Length; i++)
            {
                ...
                Marshal.Copy(n.pb, buf, 0, 0x1000);
                MEMs[i].pb = buf.AsMemory();
            }
    ...
    public static class vmm
    {
        ...
        // Modified by Frost
        public static unsafe Span<byte> MemRead(uint pid, ulong qwA, uint cb, uint flags = 0)
        {
            uint cbRead;
            Span<byte> data = new byte[cb];
            fixed (byte* pb = data)
            {
                ...
                if (cbRead != cb) // partial read
                {
                    return data.Slice(0, (int)cbRead);
                }
                return data;
            }
        }
